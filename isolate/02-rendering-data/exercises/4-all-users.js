...
const renderAllUsers = (arrOfUsers, id, classList = []) => {
  ...
  ulEl.id = id;
  for (const singleClass of classList) {
    ulEl.classList.add(singleClass);
  }

  for (const singleUser of arrOfUsers) {
    const liEl = document.createElement('li');
    liEl.innerHTML = `${singleUser.screenName}: ${singleUser.realName}`;
